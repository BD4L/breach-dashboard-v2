...
Checked against official GitHub documentation on September 12, 2026. The source is in the public `BD4L/breach-dashboard-v2` repository; scheduled collection and Pages deployment use a separate, repository-guarded workflow.
...
The Free plan supports 20 concurrent standard hosted jobs, including at most five macOS jobs, and an individual hosted job can run for at most six hours. The validation job has a 15-minute timeout and cancels superseded validation runs. The collector runs at most eight source jobs at once, with 120-second deadlines for restored tables and feeds, 600 seconds for existing PDF/browser collectors, and 12-minute job caps. The merge/build job has a 20-minute cap and Pages deployment a 10-minute cap. [Actions limits](https://docs.github.com/en/actions/reference/limits)

Schedules run from the default branch, may be delayed or dropped under load, and are disabled in public repositories after 60 days without repository activity. They cannot guarantee a precise notification deadline. Full collection is requested at :17 and :47 UTC, with a smaller recent-source pass at :02 and :32. Manual dispatch supports `all`, `recent`, or one source. The dashboard marks publication older than 90 minutes as delayed. September 12 run metadata showed gaps exceeding three hours even while delivered runs finished in about two to three minutes: adding cron expressions cannot guarantee delivery. [Scheduled workflows](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule)

GitHub Free includes 500 MB of artifact storage, shared with other applicable account usage. Successful collection deployments delete only their own consumed `source-*` and `github-pages` artifacts after public history is saved and Pages reports success. Logs, workflow summaries and durable source-run evidence remain. Interrupted runs retain transport artifacts for at most one day. This avoids multiplying a full day of artifacts as coverage and refresh frequency grow; cleanup failures still need attention. Dependency caches and other repositories have separate usage to monitor. [Storage allowances](https://docs.github.com/en/actions/reference/limits)

GitHub-hosted jobs are ephemeral. The local SQLite file cannot simply be left on a runner between jobs. The `collection-state` branch stores **public** normalized records and full revision history as diffable JSON Lines, protected by checksums. A shared workflow concurrency group serializes writes. Cache is used only for dependencies. Source and Pages artifacts are short-lived transport data; the branch is the durable source of truth. A corrupt or missing branch fails restore instead of resetting history. Archive files have a 90 MB cap and combined state a 200 MB cap; reaching either requires a reviewed archival change, not automatic deletion. Do not put private data in a public state branch or workflow artifacts.
...
The workflow accepts GitHub's authenticated `repository_dispatch` event with type `refresh-sources` and payload `{"source":"recent"}` (or `all` / an active source ID). Unknown and reference-only IDs are rejected. An existing scheduler can POST this JSON to the repository dispatch API:

```json
{"event_type":"refresh-sources","client_payload":{"source":"recent"}}
```

The credential belongs in the scheduler's secret store, never Pages or source control. The external scheduler is not configured by this change. GitHub-only timing remains best effort; an external event reduces dependence on cron delivery but does not guarantee runner capacity or upstream publication times. [Repository dispatch](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#repository_dispatch)
