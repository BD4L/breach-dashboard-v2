...
## Runner performance and cleanup — September 7, 2026

Commit `e70d50e` changes collection from four-hour intervals with three concurrent workers to 30-minute intervals with eight. The longest-running sources lead the matrix, and dependency cache keys follow the installed lockfiles with separate HTTP/browser caches. Existing worker limits, one-day artifacts, and serialized history/publication remain in place. The cleanup removes five unused imports, a duplicate font import, and redundant metadata assignments; the README shrank from 1,668 to 727 words. No tracked text contained decorative emoji, and no dependencies were added.

The existing Python suite passed 240 tests with two optional live-browser checks skipped; all 35 frontend tests passed. A clean frontend install, Astro check (zero errors/warnings, one existing hint), project-path build, public-boundary check, and actionlint 1.7.12 passed. [Hosted validation 34141193994](https://github.com/BD4L/breach-dashboard-v2/actions/runs/34141193994) also passed. Existing upstream build deprecation notices remain.

[All-source run 34141224307](https://github.com/BD4L/breach-dashboard-v2/actions/runs/34141224307) completed collection and Pages publication in 195 seconds versus 315 seconds for the preceding run: an observed 38% reduction, not a controlled benchmark or scheduling guarantee. Source response and setup times vary. SEC encountered HTTP 500 after 23 of 45 search pages; its accepted records were retained and its incomplete window remained visible. North Dakota's unverified replacement and other documented coverage limits remain.

The published snapshot at `2026-09-07T16:03:46Z` contains 27,909 reports, including two new Montana reports. Live index/archive hashes, sizes, counts, source metadata, and frontend contracts agree. Durable state `e5e194722b5fbaebed331f368bad9da0d2b01b95` restores with valid checksums and SQLite integrity: every prior report identity and first-seen time, all 28,066 prior full revisions, and all 228 prior source-run/evidence rows are preserved exactly. The new state contains 28,068 revisions and 246 source runs.

[SEC-only retry 34141652016](https://github.com/BD4L/breach-dashboard-v2/actions/runs/34141652016) received HTTP 500 on its first page. The source now explicitly reports failure; no additional retries or relaxed request limits were added. Merge and Pages deployment succeeded. The final live snapshot at `2026-09-07T16:07:23Z` preserves all 27,909 report objects exactly, with matching index/archive hashes and source metadata. SEC's upstream error remains unresolved.

## Recent monitoring and agent access, 2026-09-07

The recent-discovery phase adds source-time Latest ordering, RansomLook public claim metadata, bounded JSON/RSS feeds, a local stdio MCP connector, SEC three-day discovery with daily 30-day reconciliation, and a separate disabled staff-alert workflow. Existing official-report normalized payloads/hashes and stored history remain compatible.

Local validation: 295 Python tests completed with two opt-in browser tests skipped; 41 frontend tests and 14 MCP/feed tests passed. Astro check reported zero errors/warnings and one existing hint. Actionlint passed all five workflows with shellcheck unavailable. The live-state build produced 32,701,161 bytes inside the 50 MB site budget; public-boundary checks passed. Playwright desktop and 390 px mobile checks verified Latest, All/history loading, claim labels, source attribution, safe evidence links and no horizontal overflow. Screenshots are local ignored artifacts under output/playwright/recent-*.png.
