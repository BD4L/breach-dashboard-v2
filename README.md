...
A recent breach aggregator for research teams, with public notices, secondary news/catalog reports, explicitly unverified ransomware claims, evidence links, source health, and device-local bookmarks. Python collects records; Astro/React serves static snapshots. The default Latest view prioritizes source dates over historical imports.
...
Replace `all` with a source ID from [`ingestion/models.py`](ingestion/models.py), [`ingestion/source_catalog.py`](ingestion/source_catalog.py), or [`ingestion/additional_sources.py`](ingestion/additional_sources.py). The local CLI runs HTTP collectors sequentially; NH, NJ and SEC also have a separate [Chrome collection command](docs/local-browser-collection.md).
...
The source catalog contains all **51 configured sources** from both original directories plus RansomLook: 18 states, HHS/SEC, HIBP/Breachsense, 20 news/community feeds, five company IR pages, and three reference datasets. **48 are collection targets**; CISA KEV, NVD and Privacy Rights remain visible references with explicit reasons for exclusion from breach counts. See the [complete inventory and verified limitations](docs/source-inventory-2026-09-12.md).

GitHub Actions requests all 48 collection targets at :17 and :47 UTC. An additional recent-source pass at :02 and :32 checks 27 priority official, metadata and news sources. This creates roughly 15-minute refresh opportunities for that group when GitHub delivers the events; it is not a latency guarantee. Up to eight independent collectors run concurrently. The new HTTP collectors use a 60-second request budget inside a 120-second process deadline. Existing PDF/browser collectors retain their 600-second worker deadlines and 12-minute job caps. Failures do not cancel other sources. NH, NJ and SEC use headed Chrome on standard Ubuntu runners; Wisconsin uses standard macOS; remaining collectors use HTTP.
...
Run **Collect public sources and publish** in Actions to collect `all`, `recent`, or one active source manually. **Publish preserved public history** rebuilds Pages without scraping. GitHub may delay or drop scheduled runs; this is periodic collection, not guaranteed immediate monitoring. See [GitHub Free limits and storage](docs/github-free.md).

The merge job restores checksummed public history from the `collection-state` branch, applies valid results transactionally, and saves state before building Pages. Missing or failed sources retain prior records. Full revisions remain in durable state; caches contain dependencies and artifacts are transport only and are removed after durable persistence and successful deployment; interrupted runs retain them for at most one day. A shared concurrency group serializes collection, history writes, and publication. Partial coverage or a failed source keeps the workflow red even when retained data publishes successfully.

The visible browser checks for a published snapshot every five minutes, when returning to the tab, and on manual refresh. Browser refresh does not trigger collection. A snapshot older than 90 minutes is marked “Publication delayed”; the Sources view separates the last attempt, last usable collection, last complete run and latest report date. Failed refreshes retain the last valid snapshot. The first load includes up to 200 reports; full searches, filters, saved reports, and downloads load the archive on demand. See [snapshot compatibility](frontend/SNAPSHOTS.md).

**Today · official only** resets other filters and shows official notices and filings dated today UTC, excluding both unverified claims and secondary news/catalog reports. Checkbox dropdowns support multiple sources, report types, affected-count ranges and statuses, with Select all and Clear. Choices within one dropdown match any selected option; different dropdowns combine. Search can target organization names, report IDs, summaries, data involved and source names. Source options also have their own search. Selecting no options produces no matches.
...
- Have I Been Pwned uses its public catalog API and feed, attributed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Its account totals are not presented as numbers of people. Breachsense provides public, unverified claim metadata. News collectors retain headlines, source dates and original links; keyword matches can include incident-related reporting rather than a newly announced breach. All remain distinct from official notices in filters, RSS, JSON and MCP.
...
- Coverage varies by source and collection window. See the [current source inventory](docs/source-inventory-2026-09-12.md), [September 7 baseline audit](docs/source-status-2026-09-07.md), [coverage findings](docs/current-coverage.md), [source rediscovery](docs/collector-rediscovery.md), and [hosted access evidence](docs/source-access-diagnosis.md).
